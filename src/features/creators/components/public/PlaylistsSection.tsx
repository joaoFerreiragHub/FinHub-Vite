...
import { CreatorFull } from '@/features/creators/types/creator'
import { PlaylistResolved } from '@/features/creators/types/content'
...
const PlaylistsSection: React.FC<PlaylistsSectionProps> = ({
  ...
  const filteredPlaylists = playlists.filter((playlist) => {
    ...
    const isNotMainRegular = !(isMainPlaylistRegularAndSelected && playlist.type === 'regular')
  ...
  return (
    <>
      {filteredPlaylists.map((playlist, index) => (
        <section key={playlist.id || index} className="mb-10 space-y-4 playlist-section">
          <h2 className="text-xl font-semibold playlist-title">{playlist.title}</h2>
          <Carousel>
            {playlist.videos?.map((video, videoIndex) => {
              const videoId = typeof video === 'string' ? video : (video.videoId as string)

              return (
                <div key={videoId} className="min-w-[300px] max-w-[500px] flex-shrink-0">
                  <iframe
