    ...
    <div className="grid md:grid-cols-2 gap-6">
      {courses.map((course) => (
        <Card key={course._id} className="p-4 space-y-2">
          <img
            src={typeof course.bannerImage === 'string' ? course.bannerImage : '/placeholder.jpg'}
          ...
          <p className="text-sm font-medium">Preço: €{course.price.toFixed(2)}</p>
          ...
          <div className="flex justify-end gap-2 pt-2">
            <Button variant="ghost" onClick={() => onToggleVisibility(course._id)}>
              {course.hidden ? (
                <>
