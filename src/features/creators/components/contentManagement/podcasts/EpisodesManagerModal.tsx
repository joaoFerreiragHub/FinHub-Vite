...
import { PodcastEpisode } from '~/features/hub/types/podcastEpisode'
...
export default function EpisodesManagerModal({ podcast, open, onClose }: Props) {
  ...
  const handleToggleVisibility = (id: string) => {
    setEpisodes((prev) => prev.map((e) => (e.id === id ? { ...e, hidden: !e.hidden } : e)))
  }
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-lg">
        ...
        <div className="space-y-3">
          {episodes.map((e) => (
            <div key={e.id} className="flex justify-between items-center border p-2 rounded">
              <div className="flex-1">
                <span className="text-sm">{e.title}</span>
                {e.hidden && <span className="ml-2 text-xs text-muted-foreground">(oculto)</span>}
              </div>
