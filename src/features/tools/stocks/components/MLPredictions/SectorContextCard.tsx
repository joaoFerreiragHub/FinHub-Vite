        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-white rounded-lg p-4 border border-indigo-100 text-center">
              ...
              >
                {sectorPerformance > 0 ? '+' : ''}
                {formatNumber(sectorPerformance)}%
              </div>
            </div>
            <div className="bg-white rounded-lg p-4 border border-indigo-100 text-center">
              <div className="text-sm text-gray-500">P/E do Setor</div>
              <div className="text-lg font-bold text-blue-600">{formatNumber(sectorPE)}x</div>
            </div>
            <div className="bg-white rounded-lg p-4 border border-indigo-100 text-center">
              <div className="text-sm text-gray-500">Peers Analisados</div>
              <div className="text-lg font-bold text-purple-600">{peersCount}</div>
            </div>
          </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {sectorConfig.keyMetrics.primary.map((metric: string, idx: number) => (
                  <div
                    key={idx}
                    className="bg-white rounded p-2 border border-indigo-100 text-center"
                  >
          ...
          {!sectorConfig && (
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 text-center">
              <p className="text-sm text-gray-600">📊 Dados do setor não disponíveis</p>
            </div>
          )}
        </CardContent>
      )}
