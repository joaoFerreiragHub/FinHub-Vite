...
export function useMLPredictionsCache(enabled: boolean = true) {
  ...
  const getCachedData = useCallback(
    (symbol: string, fullAnalysis: boolean): CacheResult | null => {
  ...
  const clearSymbolCache = useCallback((symbol: string) => {
    const keysToDelete = Array.from(cacheRef.current.keys()).filter((key) => key.includes(symbol))
  ...
  const getCacheStats = useCallback(() => {
    ...
    return {
      ...
      oldestEntry:
        entries.length > 0 ? Math.min(...entries.map(([, entry]) => entry.timestamp)) : null,
    }
  }, [])
