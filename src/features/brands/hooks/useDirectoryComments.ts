...
import {
  directoryCommentsService,
  type DirectoryCommentSort,
} from '../services/directoryCommentsService'

const PAGE_SIZE = 10

interface UseDirectoryCommentsOptions {
  enabled?: boolean
...
export function useDirectoryComments(targetId: string, options: UseDirectoryCommentsOptions = {}) {
  const queryClient = useQueryClient()
  const [sortBy, setSortByState] = useState<DirectoryCommentSort>('recent')
  const normalizedTargetId = targetId.trim()
  const isEnabled = (options.enabled ?? true) && normalizedTargetId.length > 0

  const commentsQuery = useInfiniteQuery({
    queryKey: ['directory-comments', normalizedTargetId, sortBy, options.currentUserId ?? null],
    queryFn: ({ pageParam }) =>
      directoryCommentsService.getCommentTree(normalizedTargetId, {
  ...
  const invalidateComments = async () => {
    const invalidations: Array<Promise<unknown>> = [
      queryClient.invalidateQueries({
        queryKey: ['directory-comments', normalizedTargetId],
      }),
    ]
  ...
  const createCommentMutation = useMutation({
    mutationFn: ({ content, parentCommentId }: CreateCommentInput) =>
      directoryCommentsService.createComment(normalizedTargetId, content, parentCommentId),
    onSuccess: async () => {
  ...
  const updateCommentMutation = useMutation({
    mutationFn: ({ commentId, content }: { commentId: string; content: string }) =>
      directoryCommentsService.updateComment(commentId, content),
    onSuccess: async () => {
      await invalidateComments()
    },
  })
  ...
  const deleteCommentMutation = useMutation({
    mutationFn: (commentId: string) => directoryCommentsService.deleteComment(commentId),
    onSuccess: async () => {
      await invalidateComments()
    },
  })
  ...
  const likeCommentMutation = useMutation({
    mutationFn: (commentId: string) => directoryCommentsService.toggleLike(commentId),
    onSuccess: async () => {
      await invalidateComments()
    },
  })

  const setSortBy = (nextSort: DirectoryCommentSort) => {
