...
import { type Rating, type RatingListResponse } from '../../types'
...
export interface RatingListProps extends HTMLAttributes<HTMLDivElement> {
  ...
  /**
   * Props extras para todos os RatingCard
   */
  cardProps?: Partial<RatingCardProps>
  /**
   * Props por card (dinamico por rating)
   */
  getCardProps?: (rating: Rating) => Partial<RatingCardProps>
  /**
  ...
  if (response.items.length === 0 && !isLoading) {
    return (
      <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-muted-foreground/25 bg-muted/10 p-12 text-center">
        ...
        <h3 className="mb-2 text-lg font-semibold">Sem avaliacoes ainda</h3>
        <p className="text-sm text-muted-foreground">S� o primeiro a avaliar este conteudo.</p>
      </div>
    )
  }
    ...
    <div className={cn('space-y-4', className)} {...props}>
      {onSortChange && (
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-semibold">Avaliacoes ({response.total})</h3>
      ...
      <div className="space-y-4">
        {response.items.map((rating) => (
          <RatingCard
            ...
            rating={rating}
            {...cardProps}
            {...(getCardProps ? getCardProps(rating) : undefined)}
          />
        ))}
      </div>
