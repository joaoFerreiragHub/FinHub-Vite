...
export default function CourseManagementPage() {
  ...
  const handleToggleCourseVisibility = (id: string) => {
    setCourses(prev =>
      prev.map(course =>
        course._id === id ? { ...course, hidden: !course.hidden } : course
      )
    )
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <Button onClick={() => { setIsDialogOpen(true); setEditingCourse(null) }}>
      ...
      <div className="flex gap-4 border-b pb-2">
        ...
        <button onClick={() => setSelectedTab("escondidos")} className={selectedTab === "escondidos" ? "font-bold" : ""}>Ocultos</button>
      </div>

      <CoursesList
        courses={filteredCourses}
        onEdit={(course) => { setEditingCourse(course); setIsDialogOpen(true) }}
