...
export function MLPredictions({ symbol, showFullAnalysis = true }: MLPredictionsProps) {
  ...
  const [expandedSections, setExpandedSections] = useState<Set<string>>(new Set(['earnings', 'priceTarget']))
...
function ErrorState({ error, onRetry, symbol }: { error: string, onRetry: () => void, symbol: string }) {
  return (
    <Card className="border-red-200 bg-red-50">
      <CardContent className="p-8 text-center">
        ...
        <p className="text-red-600 mb-4">
          Não foi possível carregar as previsões para {symbol}
        </p>
    ...
    <Card className="border-gray-200">
      <CardContent className="p-8 text-center">
        ...
        <p className="text-gray-500 mb-4">
          Ainda não temos previsões suficientes para {symbol}
        </p>
        ...
        <div className="flex justify-center space-x-3">
          <Button variant="outline" asChild>
            <a href={`#/stocks/${symbol}/fundamentals`}>
              Ver Dados Fundamentais
            </a>
          </Button>
          <Button variant="outline" asChild>
            <a href="#/ml/coverage">
